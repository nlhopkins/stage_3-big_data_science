...
___
# The Data
There are `r length(list.files("data/data-raw"))` files containing data obtained from the assays.
```{r functions}
# Source functions
...
BLASTp`r Cite (refs, c("Abdeldaim2013-le", "Johnson2008-rl", "McGinnis2004-qg"))` was used to identify homologues of MuGam within Escherichia coli and Haemophilus influenzae. Significant sequences were aligned in COBALT `r Cite(refs, "Papadopoulos2007-zl")` and ClustalOmega`r Cite(refs, "Sievers2011-xs")` to yield the optimised alignment between MuGam, E. coli Gam and H. influenzae Gam (fig \@ref(fig:align)). Prophage regions of H. influenzae were interrogated in PHASTER `r Cite(pkgs, "Arndt2016-bg")`, identifying two prophage regions, one of which was intact and derived from Enterobacteria phage Mu (Fig. \@ref(tab:id)) and contained homologues of MuA, MuB, and MuGam (Table \@ref(tab:id)). The H. influenzae homologues are potentially polycistronic, as they are oriented in the same way as Mu phage
...
sec_footprint <- as.numeric(round((a254_mw - a280_mw) / 660, digits = 2))
...
Alignment in BLAST `r Cite (refs, c("Abdeldaim2013-le", "Johnson2008-rl", "McGinnis2004-qg"))` showed no significant similarity between Human Ku with H. influenzae, nor MuGam. However, *D. vulgaris* Mu shared a similarity of ~27% with both *H. influenzae* and MuPhage (Table \@ref(tab:blast). This information would advocate for DvGam as a more appropriate than Ku for developing a homology model of the crystal structure of HiGam.
...
data.frame(
  ...
  "sds_page" = c(
    sds$actual_weight[2],
    sds$actual_weight[2] * 2,
    sds$actual_weight[6],
    sds$actual_weight[6] * 2
  ),
...
data.frame(
  ...
  "EMSA" = c(emsa_footprint, emsa_footprint * 2),
  "SEC" = c(NA, sec_footprint)
) %>%
  knitr::kable(
