...
Cell lysates[15] of two His-tagged[16] E. coli HiGam proteins, His$_{13}$-HiGam and His$_7$-Im9-HiGam, were fractionated into soluble and insoluble protein. They were then analysed by SDS-PAGE and direct Western blotting (Fig. \@ref(fig:sds-fig)) to detect His-tagged protein within the fractions. Fractions were consistent with their expected molecular weights[17,18] (Table \@ref(tab:sds-tab)). His$_7$-Im9-HiGam was determined to have the best solubility for overproducing HiGam.
...
a280_elution <- format(round(sec_elution("a280"), digits = 2), scientific = FALSE)
a254_elution <- format(round(sec_elution("a254"), digits = 2), scientific = FALSE)
a280_mw <- sec_mw("a280")
a254_mw <- sec_mw("a254")

sec_footprint <- round((a254_mw - a280_mw) / 660, digits = 2)
...
Our SEC analysis was performed by lysing cells that Im9-HiGam with a DNase buffer. Samples were measured at two absorbances to detect DNA ($A_{254}$) and protein ($A_{280}$). DNA is present despite the use of DNase \@ref(fig:sec-fig). This is because, as we know from the SDS-PAGE, Im9-HiGam binds to linear DNA. Any DNA bound to Im9-HiGam is protected from DNase activity. This is reflective of the function observed in MuGam, which binds to linear DNA to protect it from exonucleases. Therefore, the peak at $A_{254}$ is the molecular weight of both DNA and Im9-HiGam. While the peak at $A_{280}$ is just protein, so we would assume that this is the molecular weight of Im9-HiGam alone. 


The molecular weight of Im9-HiGam determined by SEC is consistent with those given by ExPASy ProtParam `r Cite(refs, c("Gasteiger2005-rh", "Gasteiger2003-ks"))` and the SDS-PAGE analysis (Table \@ref(tab:mw_tab)), with a difference of approximately 4% and 6%, respectively. This is substantial evidence to propose that HiGam is a homodimer, as predicted by homology models (Fig. 9 B & C), even in the absence of solution. To further support this value, the sedimentation equilibrium analytical ultracentrifugation (seAUC) could be used as another non-destructive method of analysing the molecular weight of HiGam in solution `r Cite(refs, "Rhodes2009-uq")`.

```{r mw-tab, include = TRUE}
data.frame(
  "HiGam" = c("His13-HiGam Monomer", "His13-HiGam Dimer", "His7-HiGam Monomer", "His7-HiGam Dimer"),
  "ProtParam" = c("22491.54", "44983.08", "30626.50", "61253.00"),
  "SDS-PAGE" = c(NA, NA, NA, NA),
  "SEC" = c(NA, NA, (as.numeric(a280_mw)/2), a280_mw)
) %>% 
  knitr::kable(
    caption = "Table of",
    digits = 2,
