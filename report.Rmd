...
```{r mu-fig, out.width = "80%", fig.cap = "Replicative transposition of Mu phage in E. coli showing the following structures: the phage genome (green), the transposase binding sites (red & blue), MuA (yellow), the transposition target (black), and the E. coli genome (grey). Adapted from Fuller & Rice.", include = TRUE}
...
___
# Analysis 
Analyses of data is executed in R^`r Cite(pkgs, "base")`^. Plots are created with`ggplot2`^`r Cite(pkgs, "ggplot2")`^ and combined with `ggpubr`^`r Cite(pkgs, "ggpubr")`^, with aesthetics defined by [theme.R](theme.R) and fonts from `showtext`^`r Cite(pkgs, "showtext")`^. Tables are created using `kableExtra`^`r Cite(pkgs, "kableExtra")`^. Where applicable, are made interactive with `plotly`^`r Cite(pkgs, c("plotly", "plotly2020"))`^.
...
___
Report made with `rmarkdown`^`r Cite(pkgs, c("rmarkdown","rmarkdown2018", "rmarkdown2020"))`^.
