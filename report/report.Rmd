...
emsa <- emsa %>% 
  mutate(log = ((rf * emsa_rf) + emsa_intercept), actual_size = (10 ^ log))

emsa_footprint <- abs(500 / ((max(emsa$actual_size) - 330000) / 30626.5))
```
...
emsa_res_plot <- emsa_res %>%
  ggplot(aes(x = primer, y = residuals)) +
  geom_point(shape = 20,
             size = 3,
             colour = "black") +
